...
class InteractionsManager():
    ...
    def get_or_create_instance_by_name(self, classname):
        ...
        if classname in self.interaction_name2instance.keys():
            ...
            objs = Interaction.objects(name=classname)
            # TODO make EntitiesRegistry! to avoid such shitty code?:
            # if classname == "PrivateInfoSlottyForm":
            #     from components.interactions.models.forms_factory import SlottyFormInteraction
            #     objs = SlottyFormInteraction.objects(name=classname)
            if objs:
                intrctn = objs[0]
                self.interaction_name2instance[classname] = intrctn
                intrctn.connect_to_dataflow(self.ic)
                return intrctn
